    ...
    <div class="clearfix user" >
        ...
        <div class=" right_style r_user_style user_right">
            <div class="user_Borders clearfix">
                ...
                <c:if test="${not empty collectPageInfo.pageData}">
                <!--收藏样式-->
                <div class="Collect">
                    <ul class="Collect_list">


                        <c:forEach items="${collectPageInfo.pageData}" var="collects">
                        <li>
                            <div class="Collect_pro_name">
                                <a href="#" class="delete_Collect"></a>
                                <p class="img center"><a href="#"><img src="products/${collects.product.image}" /></a></p>
                                <p><a href="#">${collects.product.name}</a></p>
                                <p class="Collect_Standard">${collects.product.packaging}</p>
                                <p class="Collect_price">￥${collects.product.ourPrice}</p>
                            </div>
                        </li>
                        </c:forEach>
                    </ul>
                    ...
                    <div class="pages_Collect clearfix">
                        <a href="userCollectByPage?pageNumber=${collectPageInfo.pageNumber-1}&pageSize=${collectPageInfo.pageSize}" class="on" <c:if test="${collectPageInfo.pageNumber <= 1}">onclick="javascript:return false;" </c:if>>《</a>
                         <c:forEach var="i" begin="1" end="${collectPageInfo.page}">
                        <a href="userCollectByPage?pageNumber=${i}&pageSize=${collectPageInfo.pageSize}"><c:out value="${i}"></c:out></a>
                         </c:forEach>
                        <a href="userCollectByPage?pageNumber=${collectPageInfo.pageNumber+1}&pageSize=${collectPageInfo.pageSize}" class="on" <c:if test="${collectPageInfo.pageNumber >= collectPageInfo.page}">onclick="javascript:return false;" </c:if>>》</a>
                    </div>
                </div>
                </c:if>
                <c:if test="${empty collectPageInfo.pageData}">
                    <div style="color: darkorange;text-align: center;font-size: large">您当前没有收藏任何商品！</div>
                </c:if>
            </div>
        </div>
    </div>
